...
public class FunctionDefinition extends OldNode {

    private String kind;
    private String stateMutability;
    private Boolean isConstructor;

    public FunctionDefinition(JSONObject node, ArrayList<OldNode> subNodes) {
        super(node, subNodes);
    ...
    public String getCode() {
        ...
        OldNode inputParam = findSubNodes(new ArrayList<>(Arrays.asList("ParameterList")),"FunctionDefinition").get(0);
        OldNode returnParam = findSubNodes(new ArrayList<>(Arrays.asList("ParameterList")),"FunctionDefinition").get(1);
        code = code + inputParam.getCode() + ") ";
        code = code + getVisibility() + BasicGrammar.space + getStateMutability() + BasicGrammar.space ;
        ArrayList<OldNode> modifiers = findSubNodes(new ArrayList<>(Arrays.asList("ModifierInvocation")));
        for (OldNode modifierNode : modifiers) {
        ...
        OldNode block = findSubNode("Block");
