...
public class AddFor extends Mutation {
    Random rand;

    public AddFor(String type, String name, Properties parameters, List<Mutation> mutations) {
        ...
        if(super.start(ast)){
            for(String type : getAddTo()) {
                ...
                for (MyNode target : targetBlocks) {
                    ...
                    ParseResult<Statement> varUpdate = p.parseStatement( looplabel + "--;");
                    ParseResult<Expression> ble = p.parseExpression(looplabel+" >0");

                    BlockStmt bs = new BlockStmt();
                    NodeList<Expression> init = new NodeList<Expression>();
                    NodeList<Expression> update = new NodeList<Expression>();
                    init.add(varInit.getResult().get().asExpressionStmt().getExpression());
                    update.add(varUpdate.getResult().get().asExpressionStmt().getExpression());

                    ForStmt fs = new ForStmt(init, ble.getResult().get(), update, bs);
                    LabeledStmt ls = new LabeledStmt(looplabel,fs);
                    ...
                    if(getParameters().getProperty("move") != null){
                        ((ForStmt)ls.getStatement()).getBody().asBlockStmt().getStatements().addAll(((BlockStmt)target.getNode()).getStatements());
    ...
    private void addNested(LabeledStmt ls, int count, boolean randomBreakInsert, boolean randomContinueInsert, List<String> outerBlocks, Random r){
        ...
        if(count >= 0) {
            ...
            ForStmt fs = new ForStmt(init, ble.getResult().get(), update, bs);
            ls1 = new LabeledStmt(looplabel,fs);
            //System.out.println(((WhileStmt)ls1.getStatement()).getBody().toString() + " "+ ((WhileStmt)ls1.getStatement()).getBody().getClass().toString());
            ((ForStmt)ls1.getStatement()).getBody().asBlockStmt().getStatements().addAll(((ForStmt)ls.getStatement()).getBody().asBlockStmt().getStatements());
            ((ForStmt)ls.getStatement()).getBody().asBlockStmt().getStatements().clear();

            ((ForStmt)ls.getStatement()).getBody().asBlockStmt().getStatements().add(ls1);
        }
        ...
        if(randomBreakInsert && r.nextInt(10) < 4){
            ...
            ((ForStmt)ls.getStatement()).getBody().asBlockStmt().asBlockStmt().addStatement(s.getResult().get());
            //count =0;
        }
