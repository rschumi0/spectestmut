...
public class AddDoWhile extends Mutation {
        ...
        if(super.start(ast)){
            for(String type : getAddTo()) {
                ...
                for (MyNode target : targetBlocks) {
                    ...
                    //DoStmt ws = new DoStmt(null, new BooleanLiteralExpr(), new ReturnStmt());
                    ...
                    DoStmt ws = new DoStmt(bs,ble.getResult().get());
                    ...
                    if(getParameters().getProperty("move") != null){
                        ((DoStmt)ls.getStatement()).getBody().asBlockStmt().getStatements().addAll(((BlockStmt)target.getNode()).getStatements());
    ...
    private void addNested(LabeledStmt ls, int count, boolean randomBreakInsert, boolean randomContinueInsert, List<String> outerBlocks, Random r){
        ...
        if(count >= 0) {
        ParseResult<Statement> varInit = p.parseStatement("int " + looplabel + " = " + (r.nextInt(2)+1) + ";");

        BlockStmt bs = new BlockStmt();
        ParseResult<Expression> ble = p.parseExpression(looplabel+"-- >0");
        DoStmt ws = new DoStmt(bs,ble.getResult().get());
        ls1 = new LabeledStmt(looplabel,ws);
        //System.out.println(((DoStmt)ls1.getStatement()).getBody().toString() + " "+ ((DoStmt)ls1.getStatement()).getBody().getClass().toString());
        ((DoStmt)ls1.getStatement()).getBody().asBlockStmt().getStatements().addAll(((DoStmt)ls.getStatement()).getBody().asBlockStmt().getStatements());
        ((DoStmt)ls.getStatement()).getBody().asBlockStmt().getStatements().clear();

        ((DoStmt)ls.getStatement()).getBody().asBlockStmt().getStatements().add(varInit.getResult().get());
        ((DoStmt)ls.getStatement()).getBody().asBlockStmt().getStatements().add(ls1);
        }
        ...
        if(randomBreakInsert && r.nextInt(10) < 4){
            ...
            ((DoStmt)ls.getStatement()).getBody().asBlockStmt().asBlockStmt().addStatement(s.getResult().get());
            //count =0;
        }
