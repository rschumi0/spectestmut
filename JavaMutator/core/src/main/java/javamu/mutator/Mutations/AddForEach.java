...
public class AddForEach extends Mutation {
    Random rand;

    public AddForEach(String type, String name, Properties parameters, List<Mutation> mutations) {
        ...
        if(super.start(ast)){
            for(String type : getAddTo()) {
                ...
                for (MyNode target : targetBlocks) {
                    ...
                    String arryInit = "int " + looplabel + "[] = {";
                    for(int i = rand.nextInt(3); i >= 0; i--){
                        arryInit += rand.nextInt() + ",";
                    }
                    arryInit = arryInit.substring(0,arryInit.length()-1) +"};";

                    ParseResult<Statement> varInit = p.parseStatement(arryInit);
                    BlockStmt bs = new BlockStmt();

                    VariableDeclarationExpr vardecl = new VariableDeclarationExpr(com.github.javaparser.ast.type.PrimitiveType.intType(),"i"+RandomStringUtils.randomAlphanumeric(10));

                    VariableDeclarator var = new VariableDeclarator();
                    var.setName(looplabel);
                    //var.setType("Object");

                    ForEachStmt fes = new ForEachStmt(vardecl, var.getNameAsExpression(),bs);
                    LabeledStmt ls = new LabeledStmt(looplabel,fes);
                    ...
                    if(getParameters().getProperty("move") != null){
                        ((ForEachStmt)ls.getStatement()).getBody().asBlockStmt().getStatements().addAll(((BlockStmt)target.getNode()).getStatements());
    ...
    private void addNested(LabeledStmt ls, int count, boolean randomBreakInsert, boolean randomContinueInsert, List<String> outerBlocks, Random r){
        ...
        ForEachStmt fes = new ForEachStmt(vardecl, var.getNameAsExpression(),bs);
        ls1 = new LabeledStmt(looplabel,fes);
        //System.out.println(((WhileStmt)ls1.getStatement()).getBody().toString() + " "+ ((WhileStmt)ls1.getStatement()).getBody().getClass().toString());
        ((ForEachStmt)ls1.getStatement()).getBody().asBlockStmt().getStatements().addAll(((ForEachStmt)ls.getStatement()).getBody().asBlockStmt().getStatements());
        ((ForEachStmt)ls.getStatement()).getBody().asBlockStmt().getStatements().clear();

        ((ForEachStmt)ls.getStatement()).getBody().asBlockStmt().getStatements().add(varInit.getResult().get());
        ((ForEachStmt)ls.getStatement()).getBody().asBlockStmt().getStatements().add(ls1);
        }
        ...
        if(randomBreakInsert && r.nextInt(10) < 4){
            ...
            ((ForEachStmt)ls.getStatement()).getBody().asBlockStmt().asBlockStmt().addStatement(s.getResult().get());
            //count =0;
        }
